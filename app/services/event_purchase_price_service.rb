module EventPurchasePriceService
  ...
  def purchased_event_purchase_base_price(event_purchase)
  ...
  def purchased_event_purchase_processing_fees(event_purchase)
    purchased_event_purchase_total_price(event_purchase) - purchased_event_purchase_base_price(event_purchase)
  end

  def purchased_event_purchase_total_price(event_purchase)
    return Money.new(0) unless purchased_event_purchase_base_price(event_purchase).cents > 0
    (purchased_event_purchase_base_price(event_purchase) + Money.new(30)) / (1 - 0.029)
  end

  def event_purchase_base_price(event_purchase)
    ticket_price = event_purchase.ticket_purchase.product && event_purchase.ticket_purchase.charge.nil? ? event_purchase.ticket_purchase.product.price : Money.new(0)

    addons_price = Money.new(0)
    event_purchase.addon_purchases.each do |addon_purchase|
      addons_price += product_purchase_cost(addon_purchase) unless addon_purchase.charge
    end
